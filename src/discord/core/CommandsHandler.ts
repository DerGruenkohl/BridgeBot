...
import { type Client, Collection, REST, type RESTOptions, Routes } from "discord.js";
...
export class CommandsHandler<C extends Client, O = {}, A extends any[] = unknown[]> extends EventEmitter {
	 ...
	 * @param {{ REST?: RESTOptions, guildId?: string | string[] }} options
	 * @returns
	 */
	public deploy(client: Client<true>, options?: { REST?: RESTOptions; guildId?: string[] }): Promise<REST> {
		if (!client) throw new Error("MissingRequiredParameter: 'client' is required for the method.");

		if (!client.isReady()) throw new Error("DiscordClientError: The client is not ready yet.");
		...
		return new Promise(async (resolved, rejected) => {
			try {
				...
				if (options?.guildId) {
					if (typeof options.guildId === "string" && client.guilds.cache.get(options.guildId)) {
						await rest.put(Routes.applicationGuildCommands(client.user.id, options.guildId), {
							body: [...this.collection.values()].map((command) => command.structure),
						});
					} else if (Array.isArray(options.guildId)) {
						for (const guildId of options.guildId) {
							if (typeof guildId !== "string" || !client.guilds.cache.has(guildId)) continue;
